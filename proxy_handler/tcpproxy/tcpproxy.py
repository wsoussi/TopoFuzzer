        ...
        for sock in read_sockets:
            ...
            if sock == local_socket:
                if len(data):
                    ...
                    idle_counter_local += 1
                    if idle_counter_local > 10000:
                ...
                if len(data):
                    ...
                    idle_counter_server += 1
                    if idle_counter_server > 10000:
...
def main(args):
    ...
    try:
        while True:
            ...
            print('the socket is ' + str(in_socket))
            log(args.logfile, 'Connection from %s:%d' % in_addrinfo)
            proxy_thread = thread_with_trace(target=start_proxy_thread,
                                             args=(in_socket, args, in_modules,
                                                   out_modules))
            log(args.logfile, "Starting proxy thread " + proxy_thread.name)
            proxy_thread.start()
            # time.sleep(10)
            # # vprint("Slept for 10 seconds. Thread with sending socket closed", args.verbose)
            # # proxy_thread.kill()
            # proxy_thread.join()
